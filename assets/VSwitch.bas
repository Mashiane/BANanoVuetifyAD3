...
#DesignerProperty: Key: ItemKeys, DisplayName: Item Values (Key)(;), FieldType: String, DefaultValue:  , Description: Item Values
...
'Definition of variables used in this VSwitch component.
'Definition of variables used in this VSwitch component.
Sub Class_Globals
...
End Sub
'Initializes the VSwitch component.
'Initializes the VSwitch component.
...
'sDisabled = $"${mName}disabled"$
End Sub
'This builds the HTML tree based on defined properties for VSwitch
'This builds the HTML tree based on defined properties for VSwitch
...
Else
mElement = mTarget.Append($"<v-switch ref="${mName}" id="${mName}" v-for="item in ${sItems}" :disabled="item.${sItemDisabled}" :key="item.${sItemValue}" :label="item.${sItemText}" :value="item.${sItemValue}" :color="item.${sItemColor}"></v-switch>"$).Get("#" & mName)
End If
End If
'
...
End Sub
Sub UpdateVisible(C As VueComponent, b As Boolean) As VSwitch
C.SetData(sVIf, b)
C.SetData(sVShow, b)
Return Me
End Sub
Sub UpdateVisibleOnApp(C As VuetifyApp, b As Boolean) As VSwitch
C.SetData(sVIf, b)
C.SetData(sVShow, b)
Return Me
End Sub
...
C.SetData(sRequired, b)
End Sub
Sub UpdateReadOnlyOnApp(C As VuetifyApp, b As Boolean)
C.SetData(sReadonly, b)
End Sub
...
Return res
End Sub
Sub SetValue(C As VueComponent, b As Boolean)
If b Then
C.SetData(sVModel, sTrueValue)
Else
C.SetData(sVModel, sFalseValue)
End If
End Sub
Sub SetValueOnApp(C As VuetifyApp, b As Boolean)
If b Then
C.SetData(sVModel, sTrueValue)
Else
C.SetData(sVModel, sFalseValue)
...
End Sub
'Clear Items
Sub ClearOnApp(C As VueComponent)
xitems.Initialize
C.SetData(sItems, C.NewList)
End Sub
'add items
...
End Sub
'set checked items
Sub SetCheckedOnApp(C As VuetifyApp, vItems As List)
C.SetData(sVModel, vItems)
End Sub
'get checked items
