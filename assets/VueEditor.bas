...
'Definition of variables used in this VueEditor component.
Sub Class_Globals
...
End Sub
'Initializes the VueEditor component.
...
'sDisabled = $"${mName}disabled"$
End Sub
'This builds the HTML tree based on defined properties for VueEditor
...
C.SetData(sVModel, s)
End Sub
Sub UpdateValue(C As VueComponent, s As String)
C.SetData(sVModel, s)
End Sub
...
C.SetData(sVModel, b)
End Sub
Sub SetValueOnApp(C As VuetifyApp, s As String)
C.SetData(sVModel, s)
End Sub
Sub UpdateValueOnApp(C As VuetifyApp, s As String)
C.SetData(sVModel, s)
End Sub
...
End Try
End Sub
Sub FocusOnApp(C As VuetifyApp)
Try
Dim refs As BANanoObject = C.refs
Dim q As BANanoObject = refs.GetField(mName).GetField("quill")
q.RunMethod("focus", Null)
