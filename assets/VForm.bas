...
#DesignerProperty: Key: ReadMe, DisplayName: ReadMe*, FieldType: String, DefaultValue: The VForm is used when creating forms. When used with VField it becomes the recommended choice to create your forms. The VForm can be linked to both the VDataTable and DataSource. You can also use your own tag e.g div or v-container , Description: The id of the element to place this into
#DesignerProperty: Key: ParentID, DisplayName: ParentID, FieldType: String, DefaultValue: , Description: The id of the element to place this into. If inside a dialog, type in the dialog name and [cardtext]
...
#DesignerProperty: Key: AutoComplete, DisplayName: Auto Complete, FieldType: Boolean, DefaultValue: False, Description: Form should be Auto Complete
#DesignerProperty: Key: LazyValidation, DisplayName: LazyValidation, FieldType: Boolean, DefaultValue: True, Description: Form should be lazy validated
#DesignerProperty: Key: SubmitPrevent, DisplayName: SubmitPrevent, FieldType: Boolean, DefaultValue: True, Description: Submit Prevent to prevent reload
#DesignerProperty: Key: TableName, DisplayName: Table Name*, FieldType: String, DefaultValue: , Description: The name of the database table for CRUD
#DesignerProperty: Key: RecordSource, DisplayName: Record Source*, FieldType: String, DefaultValue: , Description: The name of the Record Source. This is used for data binding e.g. category
#DesignerProperty: Key: Singular, DisplayName: Singular*, FieldType: String, DefaultValue: , Description: Singular Name of single Record in the table e.g. category
#DesignerProperty: Key: Plural, DisplayName: Plural*, FieldType: String, DefaultValue: , Description: Plural Name of many Records in the table e.g. categories
#DesignerProperty: Key: PrimaryKey, DisplayName: Primary Key Field*, FieldType: String, DefaultValue: , Description: The name of the primary key field in the table
#DesignerProperty: Key: AutoIncrement, DisplayName: Auto Increment Field, FieldType: String, DefaultValue: , Description: The name of the auto increment field in the table
#DesignerProperty: Key: DisplayField, DisplayName: Delete Display Field*, FieldType: String, DefaultValue: , Description: The name of the field to display to confirm deletes
#DesignerProperty: Key: FocusOn, DisplayName: FocusOn, FieldType: String, DefaultValue: , Description: The name of the component to focus on when the dialog is opened
...
#DesignerProperty: Key: Disabled, DisplayName: Disabled Form, FieldType: Boolean, DefaultValue: False, Description: Form is Disabled
#DesignerProperty: Key: Readonly, DisplayName: Readonly Form, FieldType: Boolean, DefaultValue: False, Description: Form is Readonly
#DesignerProperty: Key: ShowGridDesign, DisplayName: Show Grid Design, FieldType: Boolean, DefaultValue: False, Description: Show Grid Design Border
'
#DesignerProperty: Key: OwnTag, DisplayName: Own Tag, FieldType: String, DefaultValue: , Description: Specify Own Tag e.g v-container or div
#DesignerProperty: Key: Align, DisplayName: Vertical Align, FieldType: String, DefaultValue: normal, Description: Vertical Align content, List: normal|start|center|end|baseline|stretch
#DesignerProperty: Key: Justify, DisplayName: Horizontal Align, FieldType: String, DefaultValue: normal, Description: Justify content, List: normal|start|center|end|space-between|space-around
#DesignerProperty: Key: Fluid, DisplayName: Fluid (V-Container), FieldType: Boolean, DefaultValue: false, Description: The v-container should be fluid
#DesignerProperty: Key: DFlex, DisplayName: DFlex (V-Container), FieldType: Boolean, DefaultValue: false, Description: DFlex
#DesignerProperty: Key: TextCenter, DisplayName: Text Center, FieldType: Boolean, DefaultValue: false, Description: Text Center Content
#DesignerProperty: Key: FillHeight, DisplayName: Fill Height, FieldType: Boolean, DefaultValue: false, Description: Fill Height
#DesignerProperty: Key: BackgroundImage, DisplayName: BackgroundImage, FieldType: String, DefaultValue:  , Description: Background Image for this component
...
#DesignerProperty: Key: NoGutters, DisplayName: No Gutters (V-Container), FieldType: Boolean, DefaultValue: False, Description: No Gutters
#DesignerProperty: Key: Border, DisplayName: Border, FieldType: String, DefaultValue:  , Description: Complete Border
#DesignerProperty: Key: BorderColor, DisplayName: Border Color, FieldType: String, DefaultValue:  , Description: Border Color, List: amber|black|blue|blue-grey|brown|cyan|deep-orange|deep-purple|green|grey|indigo|light-blue|light-green|lime|orange|pink|purple|red|teal|transparent|white|yellow|primary|secondary|accent|error|info|success|warning|none
...
'Definition of variables used in this VForm component.
Sub Class_Globals
...
Private bSubmitPrevent As Boolean
'
...
End Sub
'Initializes the VForm component.
...
'sDisabled = $"${mName}disabled"$
End Sub
'This builds the HTML tree based on defined properties for VForm
...
bSubmitPrevent = Props.getdefault("SubmitPrevent", False)
bSubmitPrevent = BANanoShared.parseBool(bSubmitPrevent)
...
If bSubmitPrevent Then
VElement.SetOnEvent(mCallBack, "submit.prevent", "")
End If
...
End If
End Sub
'bind this form to the data source, this extract content based on VFields in the VForm and builds up the datasource content
Sub BindDataSource(ds As BananoDataSource)
LinkDataSource(ds)
End Sub
'link form after CreateForm to data source. Ensure that you call BindState on the DS BEFOFE
...
ds.SchemaSetStrings(Strings)
End Sub
'add a rule for a field name
'<code>
...
End Sub
'link to the data table, ensure to execute BindState on the table after
'this extracts all contents from VFields and builds the details for the data-table
Sub BindDataTable(dt As VueTable)
LinkDataTable(dt)
End Sub
'link to the data table, ensure to execute BindState on the table after
'this extracts all contents from VFields and builds the details for the data-table
...
'
Dim sitemavatar As String = be.GetData("itemavatar")
sitemavatar = BANanoShared.parseNull(sitemavatar)
If sitemavatar <> "" Then
VC.SetData($"${sName}itemavatar"$, sitemavatar)
End If
'
HelpComment(sName)
'
...
VC.SetData($"${sName}ItemValue"$, sItemValue)
...
VC.SetData($"${sName}itemtext"$, sItemText)
...
If sFieldName <> "" Then
Defaults.Put(sFieldName, sDefaultValue)
End If
...
HelpCode($"Additional Notes...."$)
HelpCode("The Normal Text Field properties are applied based on your options.")
fBinding.Put("PersonList", True)
fBinding.Put("ColorList", False)
fBinding.Put("ItemAvatar", sitemavatar)
fBinding.Remove("Chips")
Dim pers As VAutoComplete
pers.Initialize(mCallBack, sName, sName)
pers.DesignerCreateView(mparent, fBinding)
For Each k As String In fldRules.Keys
Dim v As String = fldRules.Get(k)
pers.AddRule(v)
Next
VElement.BindVueElement(pers.VElement)
End Select
'eastablish bindings
...
''peform the form validation
'component.refs = vuetify.GetRefs
'Dim bValid As Boolean = frm.Validate(component)
'If bValid = False Then
'    vuetify.ShowSwalToastError("Please enter all required information!", 2000)
'    Return
'End If
''get the form content, we prefixed the vmodel of each element with user
''this will return a map object
'Dim Record As Map = frm.GetData(component)
...
'<code>
''peform the form validation
'Dim bValid As Boolean = frm.ValidateOnApp(vuetify)
'If bValid = False Then
'    vuetify.ShowSwalToastError("Please enter all required information!", 2000)
'    Return
'End If
''get the form content, we prefixed the vmodel of each element with user
''this will return a map object
'Dim Record As Map = frm.GetDataOnApp(vuetify)
'</code>
...
C.SetData(sRecordSource, rec)
End Sub
Sub BindState(CS As VueComponent)
VC = CS
If IsCreated = False Then
CreateForm
IsCreated = True
