...
#DesignerProperty: Key: Value, DisplayName: Value (Percentage), FieldType: String, DefaultValue: 10 , Description: Value, MinRange: 0, MaxRange: 100
...
#DesignerProperty: Key: Rotate, DisplayName: Rotate (Start Degree), FieldType: Int, DefaultValue: 0, Description: Rotates the circle start point in deg, MinRange: 0, MaxRange: 360
#DesignerProperty: Key: Size, DisplayName: Size (Diameter), FieldType: Int, DefaultValue: 32, Description: Sets the diameter of the circle in pixels, MinRange: 0, MaxRange: 500
#DesignerProperty: Key: Width, DisplayName: Width (Stroke), FieldType: Int, DefaultValue: 4, Description: Sets the stroke of the circle in pixels, MinRange: 0, MaxRange: 500
...
#DesignerProperty: Key: ShowAvatar, DisplayName: ShowAvatar, FieldType: Boolean, DefaultValue: False, Description: ShowAvatar
#DesignerProperty: Key: Avatar, DisplayName: Avatar Image, FieldType: String, DefaultValue: ./assets/sponge.png, Description: Avatar Image
...
'Definition of variables used in this VProgressCircular component.
'Definition of variables used in this VProgressCircular component.
Sub Class_Globals
...
Private xIndeterminate As String
Private xcaptionshow As String
Private bShowCaption As Boolean
Private bShowAvatar As String
Private xavatar As String
Private xshowavatar As String
...
End Sub
'Initializes the VProgressCircular component.
'Initializes the VProgressCircular component.
...
End If
sVShow = $"${mName}show"$
xIndeterminate = $"${mName}indeterminate"$
xcaptionshow = $"${mName}captionshow"$
xavatar = $"${mName}src"$
xshowavatar = $"${mName}srcshow"$
'sDisabled = $"${mName}disabled"$
End Sub
'This builds the HTML tree based on defined properties for VProgressCircular
'This builds the HTML tree based on defined properties for VProgressCircular
...
bShowAvatar = Props.GetDefault("ShowAvatar", False)
bShowAvatar = BANanoShared.parseBool(bShowAvatar)
...
Else
mElement = mTarget.Append($"<v-progress-circular ref="${mName}" id="${mName}"><span v-show="${xcaptionshow}">${sCaption}</span></v-progress-circular>"$).Get("#" & mName)
End If
'
VElement.Initialize(mCallBack, mName, mName)
VElement.TagName = "v-progress-circular"
VElement.Append($"<v-avatar id="${mName}avatar" v-show="${xshowavatar}"><img id="${mName}image" :src="${xavatar}" alt=""></img></v-avatar>"$)
...
VElement.AddAttr(":indeterminate", xIndeterminate)
VElement.SetData(xIndeterminate, bIndeterminate)
...
VElement.SetData(xcaptionshow, bShowCaption)
VElement.SetData(xshowavatar, bShowAvatar)
VElement.SetData(xavatar, sAvatar)
Private bShowAvatar As String
Private xavatar As String
Private xshowavatar As String
...
End Sub
Sub UpdateIndeterminate(C As VueComponent, b As Boolean)
C.SetData(xIndeterminate, b)
End Sub
Sub UpdateIndeterminateOnApp(C As VuetifyApp, b As Boolean)
C.SetData(xIndeterminate, b)
End Sub
'set the VModel of the progress at design time after banano loadlayout
Sub SetVModel(xModel As String, bShowPercent As Boolean)
xModel = xModel.tolowercase
VElement.AddAttr("v-model", xModel)
Dim mCaption As String = $"{{ ${xModel} }}"$
If bShowPercent Then
mCaption = mCaption & "%"
End If
VElement.Caption = mCaption
End Sub
...
Return Me
End Sub
Sub UpdateVisibleAvatar(C As VueComponent, b As Boolean)
C.SetData(xshowavatar, b)
End Sub
Sub UpdateVisibleAvatarOnApp(C As VuetifyApp, b As Boolean)
C.SetData(xshowavatar, b)
End Sub
Sub UpdateAvatar(C As VueComponent, S As String)
C.SetData(xavatar, s)
End Sub
Sub UpdateAvatarOnApp(C As VuetifyApp, S As String)
C.SetData(xavatar, s)
End Sub
Sub SetVisible(b As Boolean) As VProgressCircular
VC.SetData(sVIf, b)
VC.SetData(sVShow, b)
Return Me
End Sub
...
Return Me
End Sub
Sub UpdateVisibleCaptionOnApp(C As VuetifyApp, b As Boolean)
C.SetData(xcaptionshow, b)
End Sub
Sub UpdateVisibleCaption(C As VueComponent, b As Boolean)
C.SetData(xcaptionshow, b)
End Sub
...
Sub UpdateProgress(currentx As Long, totalx As Long)
Dim pd As Long = BANanoShared.ProgressDone(currentx, totalx)
VC.SetData(sVModel, pd)
End Sub
'update progress linear / circle using the vmodel
Sub UpdateProgressOnApp(A As VuetifyApp, currentx As Long, totalx As Long)
Dim pd As Long = BANanoShared.ProgressDone(currentx, totalx)
A.SetData(sVModel, pd)
End Sub
'clear the value on component
'Clear the contents bound by by v-model, needs BindState
'Clear the contents bound by by v-model, needs BindState
Sub Clear
VC.SetData(sVModel, 0)
End Sub
'clear the value on the app
Sub ClearOnApp(A As VuetifyApp)
A.SetData(sVModel, 0)
End Sub
Sub UpdatePercentOnApp(C As VuetifyApp, num As Int)
C.SetData(sVModel, num)
End Sub
Sub UpdatePercent(C As VueComponent, num As Int)
C.SetData(sVModel, num)
End Sub
