...
'Definition of variables used in this VCardText component.
'Definition of variables used in this VCardText component.
Sub Class_Globals
...
End Sub
'Initializes the VCardText component.
'Initializes the VCardText component.
...
End Sub
'This builds the HTML tree based on defined properties for VCardText
'This builds the HTML tree based on defined properties for VCardText
...
Else
mElement = mTarget.Append($"<v-card-text ref="${mName}" id="${mName}"></v-card-text>"$).Get("#" & mName)
End If
'
...
Return Me
End Sub
Sub UpdateVisibleOnApp(C As VuetifyApp, b As Boolean) As VCardText
C.SetData(mVIf, b)
C.SetData(mVShow, b)
Return Me
End Sub
