...
#Event: PlaceChanged (place As Map)
...
#DesignerProperty: Key: ReadMe, DisplayName: ReadMe*, FieldType: String, DefaultValue: If you use a date you need to set a watch to display the nicedate., Description: If you use a date you need to set a watch to display the nicedate.
...
'#DesignerProperty: Key: GMapAutoComplete, DisplayName: GMapAutoComplete, FieldType: Boolean, DefaultValue: False, Description: GMapAutoComplete
'#DesignerProperty: Key: GMapAutoCompleteRef, DisplayName: GMapAutoCompleteRef, FieldType: String, DefaultValue: input , Description: GMapAutoCompleteRef
...
'Definition of variables used in this VTextField component.
'Definition of variables used in this VTextField component.
Sub Class_Globals
...
Private bGMapAutoComplete As Boolean
Private sGMapAutoCompleteRef As String
...
End Sub
'Initializes the VTextField component.
'Initializes the VTextField component.
...
'sDisabled = $"${mName}disabled"$
End Sub
'This builds the HTML tree based on defined properties for VTextField
'This builds the HTML tree based on defined properties for VTextField
...
bGMapAutoComplete = Props.getdefault("GMapAutoComplete", False)
bGMapAutoComplete = BANanoShared.parseBool(bGMapAutoComplete)
sGMapAutoCompleteRef = Props.GetDefault("GMapAutoCompleteRef", "")
sGMapAutoCompleteRef = BANanoShared.parseNull(sGMapAutoCompleteRef)
...
<v-avatar class="mt=0 pt=0" style='cursor:pointer;border:1px solid black !important;' :color="${sVModel}" v-on="on" size="30">
</v-template>
<v-card>
...
End If
'
'add gmap auto complete
If bGMapAutoComplete Then
Dim sbTemplate As String = $"<gmap-autocomplete id="${mName}gmapauto" slot-ref-name="${mName}" child-ref-name="${mName}">
<v-template v-slot:input="slotProps"><v-text-field id="${mName}" ref="${mName}" v-on:listeners="slotProps.listeners" v-on:attrs="slotProps.attrs"></v-text-field></v-template>
</gmap-autocomplete>"$
If sParentID <> "" Then
sParentID = sParentID.ToLowerCase
mTarget.Initialize($"#${sParentID}"$)
...
End If
End If
If bColorPicker = False And bDatePicker = False And bTimePicker = False And bDateTimePicker = False And bGMapAutoComplete = False Then
...
If bGMapAutoComplete Then
'get the map
Dim sgma As String = $"${mName}gmapauto"$
Dim gma As VueElement
gma.Initialize(mCallBack, sgma, sgma)
'If sGMapAutoCompleteRef <> "" Then
'	gma.AddAttr("child-ref-name", sGMapAutoCompleteRef)
'End If
gma.Bind("select-first-on-enter", True)
'
Dim egma As String = $"${mName}_placechanged"$
gma.SetOnEventOwn(mCallBack, egma, "place_changed", "")
'
'initialized
Dim egmai As String = $"${mName}_initialized"$
gma.SetOnEventOwn(mCallBack, egmai, "initialized", "")
'
'If sGMapAutoCompleteRef <> "" Then
'	VElement.AddAttr("ref", sGMapAutoCompleteRef)
'End If
VElement.BindVueElement(gma)
End If
...
C.SetData(resx, res)
End Sub
Sub FormatDateTimeOnApp(C As VuetifyApp)
Dim res1 As String = C.GetData(dateModel)
Dim res2 As String = C.GetData(timeModel)
Dim xDate As String = ""
If res1 <> "" Then
...
Return res
End Sub
Sub GetDateOnApp(C As VuetifyApp) As String
Dim res As String = C.GetData(sVModel)
Return res
End Sub
'set formmated date
...
C.SetData(res, xDate)
End Sub
Sub FormatDateOnApp(C As VuetifyApp)
Dim sDate As String = C.GetData(sVModel)
If sDate = "" Then Return
Dim xDate As String = C.NiceDate(sDate)
Dim res As String = $"${sVModel}fmt"$
...
End Sub
Sub PasswordVisibleOnApp(C As VuetifyApp, b As Boolean)
C.setdata(sShowEyes, b)
End Sub
'toggle the password
...
End Sub
Sub UpdateRequiredOnApp(C As VuetifyApp, b As Boolean)
C.SetData(sRequired, b)
End Sub
...
Return Me
End Sub
Sub AddAttr(p As String, v As Object) As VTextField
VElement.SetAttr(p, v)
Return Me
End Sub
Sub AddStyle(p As String, v As String) As VTextField
VElement.AddStyle(p, v)
Return Me
End Sub
Sub RemoveAttr(p As String) As VTextField
...
Return Me
End Sub
Sub UpdateVisibleOnApp(C As VuetifyApp, b As Boolean) As VTextField
C.SetData(sVIf, b)
C.SetData(sVShow, b)
Return Me
End Sub
...
End If
End Sub
Sub SetValueOnApp(C As VuetifyApp, txt As String)
C.SetData(sVModel, txt)
If bDateTimePicker Then
Dim d As String = BANanoShared.MvField(txt,1, " ")
Dim t As String = BANanoShared.MvField(txt,2, " ")
...
End If
End Sub
Sub UpdateValue(C As VueComponent, txt As String)
C.SetData(sVModel, txt)
If bDateTimePicker Then
Dim d As String = BANanoShared.MvField(txt,1, " ")
Dim t As String = BANanoShared.MvField(txt,2, " ")
...
End If
End Sub
Sub UpdateValueOnApp(C As VuetifyApp, txt As String)
C.SetData(sVModel, txt)
If bDateTimePicker Then
Dim d As String = BANanoShared.MvField(txt,1, " ")
Dim t As String = BANanoShared.MvField(txt,2, " ")
