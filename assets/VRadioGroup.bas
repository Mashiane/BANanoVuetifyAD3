...
'Definition of variables used in this VRadioGroup component.
'Definition of variables used in this VRadioGroup component.
Sub Class_Globals
...
End Sub
'Initializes the VRadioGroup component.
'Initializes the VRadioGroup component.
...
'sDisabled = $"${mName}disabled"$
End Sub
'This builds the HTML tree based on defined properties for VRadioGroup
'This builds the HTML tree based on defined properties for VRadioGroup
...
VElement.Append($"<v-radio id="${mName}radio" v-for="(item, i) in ${sItems}" :disabled="item.${sItemDisabled}" _ :key="item.${sItemValue}" :label="item.${sItemText}" :value="item.${sItemValue}" :color="item.${sItemColor}"></v-radio>"$)
