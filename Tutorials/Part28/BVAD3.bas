...
Sub NewListItem(VC As VueComponent, elID As String, DataSource As String, Key As String, numLines As Int, props As Map)
	...
	End Select
	'
	vlistitem.SetOnOwnEvent(EventHandler, $"${elID}_click"$, "click", $"item.${Key}"$)
	'
...
'	End If
'	'
'	vbtnright.SetOnEvent(EventHandler, "click", "")
'	'
...
'	avatar.id = $"${elID}avatar"$
'	'
'	img.Src = imgURL
'	img.id = elID
'	vbadge.Color = badgeColor
'	vbadge.id = $"${elID}badge"$
'	
...
'	End If
'	
'	img.SetOnEvent(EventHandler, "click", "")
'	'
...
'	
'	mbadgebtn.SetOnEvent(EventHandler, "click", "")
'	'
...
'	End If
'	'
'	vbadgeicon.SetOnEvent(EventHandler, "click", "")
'	'
...
'	End If
'	
'	Dim btn As BANanoElement = NewButtonIcon(VC, elID, eIcon, eColor, np, iconprops)
'	Return btn
'End Sub
...
'	
'		
'	mbtn.SetOnEvent(EventHandler, "click", "")
'	'
...
'	End If
'	'
'	mbutton.SetOnEvent(EventHandler, "click", "")
'	'
...
'	vtextfield.Ref = vmodel
'	vtextfield.id = elID
'	vtextfield.SetOnEvent(eventHandler, "click:append", "")
'	vtextfield.SetOnEvent(eventHandler, "click:prepend", "")
'	vtextfield.SetOnEvent(eventHandler, "click:append-outer", "")
'	vtextfield.SetOnEvent(eventHandler, "click:prepend-inner", "")
'	
'	'
'	If banano.IsNull(props) = False Then
'		For Each k As String In props.Keys
...
'	vslider.SetOnEvent(eventHandler, "click:append", "")
'	vslider.SetOnEvent(eventHandler, "click:prepend", "")
'	vslider.SetOnEvent(eventHandler, "click", "")
'	vslider.SetOnEvent(eventHandler, "start", "")
'	vslider.SetOnEvent(eventHandler, "end", "")
'	'
'	If banano.IsNull(props) = False Then
'		For Each k As String In props.Keys
'			Dim v As Object = props.Get(k)
...
'	vfileinput.SetOnEvent(eventHandler, "change", "")
'	vfileinput.SetOnEvent(eventHandler, "click:append", "")
'	vfileinput.SetOnEvent(eventHandler, "click:prepend", "")
'	vfileinput.SetOnEvent(eventHandler, "click:append-outer", "")
'	vfileinput.SetOnEvent(eventHandler, "click:prepend-inner", "")
'	
'	'
'	If banano.IsNull(props) = False Then
'		For Each k As String In props.Keys
...
'	End If
'	'
'	vselect.SetOnEvent(eventHandler, "change", "")
'	'
...
'	End If
'	'
'	vswitch.SetOnEvent(eventHandler, "change", "")
'	'
...
'	End If
'	'
'	vrating.SetOnEvent(eventHandler, "input", "")
'	'
...
'	End If
'	'
'	vcheckbox.SetOnEvent(eventHandler, "click", "")
'	'
'	VC.BindVueElement(vcheckbox)
'	'
'	Dim ah As BANanoElement
...
'	End If
'	'
'	vradiogroup.SetOnEvent(eventHandler, "change", "")
'	'
...
'   
'	'load the layout in a temporal holder
'	Ret = banano.LoadLayoutArray("#appendholder", "vimage", True)
'	' ret returns a unique number you can use to get all views
'	AllViews = banano.GetAllViewsFromLayoutArray(Me, "vimage", Ret)
'	
'	'get the text field, there is only 1 element on the layout
'	Dim vimg As VueElement = AllViews.get("vimage")
'	vimg.Height = sheight
'	vimg.Width = swidth
'	vimg.Src = src
'	vimg.Alt = alt
...
'	'
'	vimg.SetOnEvent(eventHandler, "click", "")
'	'
'	If banano.IsNull(props) = False Then
'		For Each k As String In props.Keys
'			Dim v As Object = props.Get(k)
...
'	span.Caption = label
'	'
'	vchip.SetOnEvent(eventHandler, "click", $"'${elID}'"$)
'	vchip.SetOnEvent(eventHandler, "click:close", $"'${elID}'"$)
'	
'	'
...
'	'
'	vchipgroup.SetOnEvent(eventHandler,  "change", "")
'	'
