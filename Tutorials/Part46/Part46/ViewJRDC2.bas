...
Sub Process_Globals
	...
	Public name As String = "jrdc2page"
...
Sub Initialize
	...
	'
	vuetify.AddParagraph(Me, sqliteCont.MatrixID(1, 1), "para1", "BANanoServer jRDC2", "", "")
...
Sub btnAdd_click(e As BANanoEvent)
	Log("ViewJRDC2.btnAdd_click...")
	Dim dbMySQL As BANanoMySQLE
	dbMySQL.Initialize("test", "users", "id", "id")
	dbMySQL.SetCallBack("ViewJRDC2", "GetAllUsers")
	'add field types
	...
	dbMySQL.command = "insert_users"
	dbMySQL.NoResult = True
	'build the payload
	Dim payload As Map = dbMySQL.Build(False)
	
	'call the method from the app
	vuetify.RunMethod("JRDC2OnBrowser", payload)
End Sub
...
Sub GetAllUsers
	...
	Dim dbMySQL As BANanoMySQLE
	dbMySQL.Initialize("test", "users", "id", "id")
	dbMySQL.SetCallBack("ViewJRDC2", "GetAllUsersNext")
	dbMySQL.SelectAll(Array("*"), Array("name"))
	dbMySQL.command = "select_users"
	'build the payload
	Dim payload As Map = dbMySQL.Build(False)
	'call the method from the app
	vuetify.RunMethod("JRDC2OnBrowser", payload)
...
Sub btnRec1_click(e As BANanoEvent)
	...
	Dim dbMySQL As BANanoMySQLE
	dbMySQL.Initialize("test", "users", "id", "id")
	dbMySQL.SetCallBack("ViewJRDC2", "rec1")
	dbMySQL.SchemaAddInt(Array("id"))
	dbMySQL.Read(1)
	dbMySQL.command = "select_user"
	'build the payload
	Dim payload As Map = dbMySQL.Build(False)
	'call the method from the app
	vuetify.RunMethod("JRDC2OnBrowser", payload)
