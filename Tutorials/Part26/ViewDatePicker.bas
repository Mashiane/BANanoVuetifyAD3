...
Sub Initialize
	...
	Dim dp1 As BANanoElement = BVAD3.NewDatePicker(datepicker, "dp1",  "dob", "Date of Birth", "", True, "mdi-calendar", "Date of birth", CreateMap(":outlined":True))
	vdatepickercontainer.Matrix(1, 1).Append(dp1.GetHTML)
	'
	Dim dp2 As BANanoElement = BVAD3.NewDatePicker(datepicker, "dp2", "dod", "Date of Death", "", True, "mdi-calendar", "Date of death", CreateMap(":shaped":True, ":outlined":True))
	...
	'
	Dim tp1 As BANanoElement = BVAD3.NewTimePicker(datepicker, "tp1", "tob", "Time of Birth", "", True, "mdi-clock", "Time of birth", CreateMap(":filled":True))
	vdatepickercontainer.Matrix(2, 1).Append(tp1.GetHTML)
	'
	Dim tp2 As BANanoElement = BVAD3.NewTimePicker(datepicker, "tp2", "tod", "Time of Death", "", True, "mdi-clock", "Time of death", CreateMap(":rounded":True, ":outlined":True))
