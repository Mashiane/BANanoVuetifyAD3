...
'	
	...
	Dim txtusername1 As VueElement = vuetify.AddTextField(Me, vtextfieldcontainer.MatrixID(4,1), "txtUserName1", "username", "User Name", "", True, "mdi-account-circle", 0, "", CreateMap(":dense":True))
	textfields.BindVueElement(txtusername1)
	'
	Dim txtpassword1 As VueElement = vuetify.AddPassword(Me, vtextfieldcontainer.MatrixID(4,2), "txtPassword1", "pwd", "Password", "", True, "mdi-lock", 10, "", Null)
	textfields.BindVueElement(txtpassword1)
	'
	Dim txtsearch As VueElement = vuetify.AddTextField(Me, vtextfieldcontainer.MatrixID(4,3), "txtsearch", "search", "Search", "", False, "", 0, "", CreateMap("append-icon":"mdi-magnify",":single-line":True, ":hide-details":True))
	textfields.BindVueElement(txtsearch)
	'
	Dim r5c1 As String = vtextfieldcontainer.MatrixID(5, 1)
	Dim txtsearch50 As VueElement = vuetify.AddTextField(Me, r5c1, "txtsearch50", "searchx", "Search 50", "", False, "", 0, "", CreateMap("append-icon":"mdi-magnify",":single-line":True, ":hide-details":True))
	textfields.BindVueElement(txtsearch50)
	'
	Dim r5c2 As String = vtextfieldcontainer.MatrixID(5, 2)
	Dim txtpwd As VueElement = vuetify.AddPassword(Me, r5c2, "txtpwd", "searchp", "Password X", "", False, "mdi-lock", 10, "", Null)
	textfields.BindVueElement(txtpwd)
