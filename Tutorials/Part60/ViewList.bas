...
Sub BuildList7
	'inject a list
	Dim list7 As VueElement = vuetify.AddList(Me, vlists.MatrixID(4, 1), "list7", True, True, False, Null)
	'initialize the listview options, we use the default options
	'to source the data
	Dim list7Options As ListViewItemOptions = vuetify.NewListViewItemOptions
	'set the data source of the list items
	list7Options.dataSource = "tasks1"
	list7Options.showleftswitches = True
	'add the template to the list
	list7.AddListViewTemplate(0, list7Options)
	'bind the template
	lists.BindVueElement(list7)
	'
	list7.ClearItems
	list7.AddItemHeader("Left Switch - Tasks")
	list7.AddItemDivider(True)
	list7.AddItemLeftSwitch("1", True, "Buy 10 Bananas", "", "", "", "mdi-delete", "red", 0, "")
	list7.AddItemLeftSwitch("2", True, "Eat 2 Bananas", "", "", "", "mdi-delete", "red", 0, "")
	list7.AddItemLeftSwitch("3", True, "Count Bananas", "", "", "", "mdi-delete", "red", 0, "")
	list7.AddItemLeftSwitch("4", True, "Monkey stole 1 Bananas", "", "", "", "mdi-delete", "red", 0, "")
	list7.AddItemLeftSwitch("5", False, "Poo 2 Bananas", "", "", "", "mdi-delete", "red", 0, "")
	'
	lists.SetData("tasks1", list7.Records)
End Sub

Sub BuildList8
	'inject a list
	Dim list8 As VueElement = vuetify.AddList(Me, vlists.MatrixID(4, 2), "list8", True, True, False, Null)
	'initialize the listview options, we use the default options
	'to source the data
	Dim list8Options As ListViewItemOptions = vuetify.NewListViewItemOptions
	'set the data source of the list items
	list8Options.dataSource = "tasks2"
	list8Options.showrightswitches = True
	list8Options.switchinset = True
	'add the template to the list
	list8.AddListViewTemplate(0, list8Options)
	'bind the template
	lists.BindVueElement(list8)
	'
	list8.ClearItems
	list8.AddItemHeader("Right Switch Tasks")
	list8.AddItemDivider(True)
	list8.AddItemRightSwitch("1", True, "Buy 10 Bananas", "", "", "")
	list8.AddItemRightSwitch("2", True, "Eat 2 Bananas", "", "", "")
	list8.AddItemRightSwitch("3", True, "Count Bananas", "", "", "")
	list8.AddItemRightSwitch("4", True, "Monkey stole 1 Bananas", "", "", "")
	list8.AddItemRightSwitch("5", False, "Poo 2 Bananas", "", "", "")
	'
	lists.SetData("tasks2", list8.Records)
End Sub
